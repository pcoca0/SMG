...
selectProveedor() {
  ...
  this.suscriptions.push(this.productService.getVendorProducts(this.proveedorView.id).subscribe(
    resp =>{ this.productos = resp.data.productos;
             if (this.productos.length === 0) { this.swalService.warning('El proveedor no tiene productos asignados');}
           }
  ));
  }
  ...
  updateTotalizador() {
    ...
    this.vendorInvoiceRequest.productos.forEach( i => {
      this.totalizador = this.totalizador + (i.precioCompra * i.cantidad);
      this.iva = this.iva + (((i.precioCompra * i.cantidad) * Number(i.ivaCompra) ) / 100 );
    });
  }
      ...
      this.suscriptions.push(
        this.vendorInvoiceService.addVendorInvoice(this.vendorInvoiceRequest).subscribe(
          response => {
                        ...
                        this.router.navigate(['editarFacturaProveedor', response.data.facturasProveedores[0].id]);
                      },
